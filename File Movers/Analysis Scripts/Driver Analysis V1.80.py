...
def sc03x1 (time, target_speed, actual_speed, upper_boundary, lower_boundary, filename, filecut, driver, sheet, errorcnt1, errortime1, violationcnt1):
    ...
    filename.close()
    return
def ftp75 (time, target_speed, actual_speed, upper_boundary, lower_boundary, phase ,filename, filecut, driver, sheet, errorcnt1, errortime1, violationcnt1, errorcnt2, errortime2, violationcnt2, violationtime2, errorcnt3, errortime3, violationcnt3, violationtime3, iwr3):
    phase2start = phase.index(2)
    phase1end = phase2start - 1
    phasesoakstart = phase.index(0)
    phase2end = phasesoakstart - 1
    phase3start = phase.index(3)
    phase3end = (len(phase) - 1)
    
    phase1_time = time[0:phase1end]
    phase1_target_speed = target_speed[0:phase1end]
    phase1_actual_speed = actual_speed[0:phase1end]
    phase1_upper_boundary = upper_boundary[0:phase1end]
    phase1_lower_boundary = lower_boundary[0:phase1end]

    phase2_time = time[phase2start:phase2end]
    phase2_target_speed = target_speed[phase2start:phase2end]
    phase2_actual_speed = actual_speed[phase2start:phase2end]
    phase2_upper_boundary = upper_boundary[phase2start:phase2end]
    phase2_lower_boundary = lower_boundary[phase2start:phase2end]

    phase3_time = time[phase3start:phase3end]
    phase3_target_speed = target_speed[phase3start:phase3end]
    phase3_actual_speed = actual_speed[phase3start:phase3end]
    phase3_upper_boundary = upper_boundary[phase3start:phase3end]
    phase3_lower_boundary = lower_boundary[phase3start:phase3end]
    ...
    axs1[0].set_xlabel('Time (s)')
    axs1[0].set_ylabel('Speed (Km/h)')
    axs1[0].text(0, 108.5, 'Driver: ' + driver)
    axs1[0].text(0, 104.5, 'Error Count: ' + errorcnt1 + ',  Error Time: ' + errortime1)
    axs1[0].text(0, 100.5, 'Violation Count: ' + violationcnt1 + ',  Violation Time: ' + violationtime1 + ', IWR Phase 1: ' + iwr1 + '%')
    axs1[0].text(360, 103.5, 'IWR Test: ' + iwrtest + '%')
    axs1[0].plot(phase1_time,phase1_target_speed, label="Target Speed", linewidth= 0.75) # line width unreadable below 0.5
    axs1[0].plot(phase1_time,phase1_actual_speed, label="Actual Speed",linewidth= 0.75)
    axs1[0].plot(phase1_time,phase1_upper_boundary, linestyle= '--',linewidth= 0.75, color= 'grey')
    axs1[0].plot(phase1_time,phase1_lower_boundary, linestyle= '--',linewidth= 0.75, color= 'grey')
    ...
    axs1[0].grid(linestyle = '--', linewidth= 0.5)
    axs1[0].margins(0)
    axs1[0].set_ylim([-0.3,100])

    axs1[1].set_title('Phase 2')
    axs1[1].set_xlabel('Time (s)')
    axs1[1].set_ylabel('Speed (Km/h)')
    axs1[1].text(505, 63, 'Error Count: ' + errorcnt2 + ',  Error Time: ' + errortime2)
    axs1[1].text(505, 60.5, 'Violation Count: ' + violationcnt2 + ',  Violation Time: ' + violationtime2 + ', IWR Phase 2: ' + iwr2 + '%')
    axs1[1].plot(phase2_time,phase2_target_speed, label="Target Speed", linewidth= 0.75,)
    axs1[1].plot(phase2_time,phase2_actual_speed, label="Actual Speed", linewidth= 0.75)
    axs1[1].plot(phase2_time,phase2_upper_boundary, linestyle= '--',linewidth= 0.75, color= 'grey')
    axs1[1].plot(phase2_time,phase2_lower_boundary, linestyle= '--',linewidth= 0.75, color= 'grey')
    ...
    axs1[1].grid(linestyle = '--', linewidth= 0.5)
    axs1[1].margins(0)
    axs1[1].set_ylim([-0.3,60])
    
     #Figure 2
    fig2, axs2 = plt.subplots(1, figsize=(11.96,8.27))
    plt.subplots_adjust(left=0.05, bottom=0.06, right=0.97, top=0.93, wspace=0.2, hspace=0.23)
    fig.suptitle(filename)
    newax = fig2.add_axes([0.84, 0.881, 0.124, 0.115], anchor='NE',)
    newax.imshow(logo)
    newax.axis('off')    
    axs2.set_title('Phase 3')
    axs2.set_xlabel('Time (s)')
    axs2.set_ylabel('Speed (Km/h)')
    axs2.text(1918, 102.5, 'Error Count: ' + errorcnt3 + ',  Error Time: ' + errortime3)
    axs2.text(1918, 100.5, 'Violation Count: ' + violationcnt3 + ',  Violation Time: ' + violationtime3 + ', IWR Phase 3: ' + iwr3 + '%')
    axs2.plot(phase3_time,phase3_target_speed, label="Target Speed", linewidth= 0.75)
    axs2.plot(phase3_time,phase3_actual_speed, label="Actual Speed", linewidth= 0.75)
    axs2.plot(phase3_time,phase3_upper_boundary, linestyle= '--',linewidth= 0.75, color= 'grey')
    axs2.plot(phase3_time,phase3_lower_boundary, linestyle= '--',linewidth= 0.75, color= 'grey')
    axs2.legend()
    axs2.grid(linestyle = '--', linewidth= 0.5)
    axs2.margins(0)
    axs2.set_ylim([-0.3,100])
    ...
    filename.savefig(fig2)
    filename.close()
def WLTC_city (time, target_speed, actual_speed, upper_boundary, lower_boundary, filename, filecut, driver, sheet, errorcnt1, errortime1, violationcnt1, errorcnt2, errortime2, violationcnt2, violationtime2,iwr2):
    phase1_time = time[0:5890]
    phase1_target_speed = target_speed[0:5890]
    phase1_actual_speed = actual_speed[0:5890]
    phase1_upper_boundary = upper_boundary[0:5890]
    ...
    fig, axs1 = plt.subplots(2, figsize=(11.69,8.27))
    plt.subplots_adjust(left=0.05, bottom=0.06, right=0.97, top=0.93, wspace=0.2, hspace=0.23)
    fig.suptitle(filename,horizontalalignment='center', verticalalignment='top')
    newax = fig.add_axes([0.84, 0.881, 0.124, 0.115], anchor='NE',)
    newax.imshow(logo)
    newax.axis('off')    
    axs1[0].set_title('Phase 1')
...
if cycle == 'WLTC Class3b TYPE 1' or cycle == 'WLTC Class3b TYPE 1 Development' or cycle == 'WLTC Class3b Type1':
    ...
    iwr3 = str(round(sheet['T14'].value, 2))
    violationtime3 = str(round(sheet['T16'].value, 3))
    ftp75(time, target_speed, actual_speed, upper_boundary, lower_boundary, phase, filename, filecut, driver, sheet, errorcnt1, errortime1, violationcnt1, errorcnt2, errortime2, violationcnt2, violationtime2, errorcnt3, errortime3, violationcnt3, violationtime3, iwr3)    
elif cycle == 'MKAA KYC WLTC minus Motorway' or cycle == 'WLTC City':
    ...
    violationcnt2 = str(sheet['L16'].value)
    violationtime2 = str(round(sheet['T16'].value, 3))
    WLTC_city(time, target_speed, actual_speed, upper_boundary, lower_boundary, filename, filecut, driver, sheet, errorcnt1, errortime1, violationcnt1, errorcnt2, errortime2, violationcnt2, violationtime2,iwr2)  